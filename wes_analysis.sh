...
INTERVALS="/path/to/target_intervals.bed" # BED file for exome capture regions
BWA="/path/to/bwa"
GATK="/path/to/gatk"
THREADS=8
...
${GATK} BaseRecalibrator -R ${REF_GENOME} -I ${OUTPUT_DIR}/alignment/${SAMPLE_NAME}.dedup.bam --known-sites /path/to/known_sites.vcf -O ${OUTPUT_DIR}/alignment/${SAMPLE_NAME}.recal.table
${GATK} ApplyBQSR -R ${REF_GENOME} -I ${OUTPUT_DIR}/alignment/${SAMPLE_NAME}.dedup.bam --bqsr-recal-file ${OUTPUT_DIR}/alignment/${SAMPLE_NAME}.recal.table -O ${OUTPUT_DIR}/alignment/${SAMPLE_NAME}.recal.bam
samtools index ${OUTPUT_DIR}/alignment/${SAMPLE_NAME}.recal.bam

# Step 8: Call variants using GATK HaplotypeCaller
${GATK} HaplotypeCaller -R ${REF_GENOME} -I ${OUTPUT_DIR}/alignment/${SAMPLE_NAME}.recal.bam -L ${INTERVALS} -O ${OUTPUT_DIR}/variants/${SAMPLE_NAME}.g.vcf.gz -ERC GVCF

# Step 9: Convert GVCF to VCF using GATK GenotypeGVCFs
${GATK} GenotypeGVCFs -R ${REF_GENOME} -V ${OUTPUT_DIR}/variants/${SAMPLE_NAME}.g.vcf.gz -O ${OUTPUT_DIR}/variants/${SAMPLE_NAME}.vcf.gz

# Step 10: Index the VCF file using bcftools
bcftools index ${OUTPUT_DIR}/variants/${SAMPLE_NAME}.vcf.gz

# Step 11: Filter variants using bcftools
...
# Summary
echo "Whole-exome sequencing analysis pipeline completed successfully."
